...
from orders.payments import session
# from api.utils import order_created
...
def create_order(request, **kwargs):
    ...
    queryset, total = Cart.objects.get_cart_total(session_manager.cart_session_id)
    ...
    # items = [ProductHistory(unit_price=item.product.unit_price, product=item.product) for item in queryset]
    # for item in items:
    #     item.save()
    ...
    # order_created.send(sender=customer_order, session_manager=session_manager, products=queryset)
