<template>
  <footer class="relative w-full border-t-2 border-t-gray-50 mt-10 pt-10">
    <div class="w-full px-8 mx-auto max-w-7xl">
      <div class="grid justify-between grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div :class="`grid-cols-${numberOfItems}`" class="grid justify-between gap-4">
          <ul v-for="(item, i) in items.sections" :key="i">
            ...
            <li v-for="link in item.links" :key="link.to" :aria-label="$t(link.name)">
      ...
      <div class="flex flex-col items-center justify-between w-full py-4 mt-12 border-t border-slate-200 md:flex-row md:justify-between">
        <p class="block mb-4 text-sm text-center text-slate-500 md:mb-0">
          Copyright © 2024 
          <NuxtLink id="footer-brand-copywrite" to="/">Ecommerce</NuxtLink>. All
          Rights Reserved.
        </p>
